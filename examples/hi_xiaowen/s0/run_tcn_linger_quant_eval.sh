...
dir=/data/user/yswang/task/wekws/exp/hi_xiaowen_tcn_linger_v1_quant_eval/
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
  wb_analyse=$(basename $score_checkpoint | sed -e 's:.pt$:.wb_analyse.txt:g')
  out_analyse=$(basename $score_checkpoint | sed -e 's:.pt$:.out_analyse.txt:g')
  python wekws/bin/linger_analyse.py \
    --config $dir/config.yaml \
    --checkpoint $score_checkpoint \
